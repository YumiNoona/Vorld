...
export function LiveDemo() {
  ...
  return (
    <section className="py-24 sm:py-32 bg-[--surface] relative overflow-hidden border-t border-[--border-strong]">
      <div className="container max-w-7xl px-4 mx-auto">
        ...
        <div className="relative w-full max-w-5xl mx-auto aspect-[16/9] bg-[#0d0d0d] rounded-2xl border border-border-strong overflow-hidden shadow-2xl">
          {error ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-[--text-3] gap-4">
              <span className="text-sm">Failed to load 3D Demo. Check your connection.</span>
              <button 
                onClick={() => window.location.reload()}
                className="px-4 py-2 bg-[--accent-subtle] border border-[--accent-border] rounded-lg text-[--accent] text-xs hover:brightness-110 transition-all font-bold uppercase tracking-wider"
              >
                Retry Loading
              </button>
            </div>
            ...
            {Object.entries(MESH_DATA).map(([id, data]) => (
              <button
                ...
                className={cn(
                  ...
                  activeMesh === id 
                    ? "bg-[--accent] border-[--accent] text-[--accent-fg] shadow-glow" 
                    : "bg-[--surface-raised]/40 backdrop-blur-md border-[--border] text-[--text-2] hover:border-[--border-strong] hover:text-[--text-1]"
                )}
              >
                <div className={cn("w-2 h-2 rounded-full", activeMesh === id ? "bg-white" : "bg-[--text-3]")} />
                {data.label}
              </button>
            ))}
          </div>
          ...
          <AnimatePresence>
            {showInfo && activeMesh && MESH_DATA[activeMesh] && (
              <motion.div
                ...
                exit={{ opacity: 0, x: 20 }}
                className="absolute top-6 right-6 w-72 p-6 rounded-xl bg-[--surface]/80 backdrop-blur-xl border border-[--border-strong] shadow-2xl"
              >
                ...
                <h4 className="text-lg font-medium text-[--text-1] mb-2">
                  {MESH_DATA[activeMesh].label}
                </h4>
                <p className="text-sm text-[--text-2] leading-relaxed mb-6">
                  {MESH_DATA[activeMesh].description}
                </p>
                <button className="w-full h-10 bg-[--accent] hover:brightness-110 text-[--accent-fg] text-sm font-semibold rounded-lg transition-all duration-200">
                  Learn more
                </button>
              </motion.div>
            )}
          ...
          {/* Hint Overlay */}
          {!activeMesh && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 text-[--text-3] bg-[--bg]/20 backdrop-blur-sm px-4 py-2 rounded-full">
